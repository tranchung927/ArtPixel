...
public class Canvas: UIView {
    ...
    func setupDisplayCanvasView() {
        ...
        for x in 0..<self.numberOfItemInColumn {
            self.pixels.append([])
             print(self.symbolsMatix[x].count)
            for y in 0..<self.numberOfItemInRow {
                let label = UILabel()
                label.frame = CGRect(x: CGFloat(x) * itemSize, y: CGFloat(y) * itemSize, width: itemSize, height: itemSize)
                label.backgroundColor = UIColor.white
                label.text = self.symbolsMatix[x][y]
